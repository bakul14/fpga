...
brew install verilator verible cmake clang-format coreutils
```
...
```bash
sudo apt update -y
sudo apt install -y build-essential cmake git gtkwave clang-format autoconf flex bison help2man software-properties-common
...
При компиляции тестов задачи создается файл `build/<задача>/compile_commands.json` с флагами компиляции и файл `.clangd` рядом с исходниками, указывающий на этот файл. У каждой задачи свой контекст компиляции, так что они не мешают друг другу.

В Zed автодополнение работает сразу, в VS Code нужно расширение **clangd** от LLVM, которое предлагается установить при открытии проекта.
...
Если не нравится `gtkview` из-за слишком мелкого интерфейса, то в VS Code можно установить расширение **WaveTrace** для просмотра `.vcd`-файлов и использовать его вместо `gtkview`.

Отдельно и только на Mac OS можно установить `surfer` для просмотра `.vcd`-файлов (самый удобный кмк):
