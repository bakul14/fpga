...
В Zed автодополнение работает сразу, в VS Code нужно расширение **clangd** от LLVM, которое предлагается установить при открытии проекта. Расширение **C/C++** от Microsoft ставить не надо: оно даёт собственное автодополнение, которое конфликтует с clangd, поэтому в настройках проекта его IntelliSense отключён.
